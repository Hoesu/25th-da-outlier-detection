...
class VAE(nn.Module):
    ...
    def encode(self, x):
        ...
        - Purpose:
            - returns encoded_mu and encoded_logvar for variational layer, output_encoder for VSAM layer.
        """
    ...
    def forward(self, x):  
        ...
        if self.training:
            ...
            return output, mu, logvar, output_encoder, state_decoder, c_det, z, c_final, c_t_mus, c_t_logvars
        # If not training (inference mode), sample L times from z and generate multiple reconstructions
            ...
            for l in range(self.sample_reps):
                ...
                if l == 0:
                    ...
                    output_mu, output_logvar, state_decoder = self.decode(z, c_final)
                    output_mu_all = output_mu.unsqueeze(0)              # (1, seq_size, batch_size, input_size)
                    output_logvar_all = output_logvar.unsqueeze(0)  # (1, seq_size, batch_size, input_size)
                    output = output_mu  # Store the first output
                else:
                    # Decode and concatenate subsequent outputs
                    output_mu, output_logvar, state_decoder = self.decode(z, c_final)
                    output_mu_all = torch.cat((output_mu_all, output_mu.unsqueeze(0)), 0)         # (L, seq_size, batch_size, input_size)
                    output_logvar_all = torch.cat((output_logvar_all, output_logvar.unsqueeze(0)), 0) # (L, seq_size, batch_size, input_size)
            # Return the final output and all the sampled reconstructions
            return output, mu, logvar, output_encoder, state_decoder, c_det, z, c_final, c_t_mus, c_t_logvars, output_mu_all, output_logvar_all